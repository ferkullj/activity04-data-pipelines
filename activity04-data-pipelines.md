...
**Response**: When you select message equals TRUE, the console output is
included in the knit output document
...
**Response**: Setting the message option to TRUE shows the console
output with column specification
...
**Response**: I would need the variables ‘major’ and
‘unemployment\_rate’ to answer this. I could order the majors by
unemployment\_rate to find which one has the lowest rate.
...
**Response**: Nuclear Engineering is the major with the highest
unemployment rate
...
**Response**: Early Childhood Education has the highest proportion of
women.
...
``` r
college_recent_grads %>% 
  filter(major == stem_categories %in% c("Biology & Life Science",
                     "Computers & Mathematics",
                     "Engineering",
                     "Physical Sciences") & median < 36000) %>%
  select(major, p25th, median, p75th)
```

    ## Warning in major == stem_categories %in% c("Biology & Life Science", "Computers
    ## & Mathematics", : longer object length is not a multiple of shorter object
    ## length

    ## # A tibble: 0 x 4
    ## # … with 4 variables: major <chr>, p25th <dbl>, median <dbl>, p75th <dbl>
