...
**Response**: When you select message equals TRUE, the console output is included in the knit output document
...
**Response**: Setting the message option to TRUE shows the console output with column specification
...
```{r college_grads_top, message=TRUE}
college_recent_grads
```
...
**Response**: I would need the variables 'major' and 'unemployment_rate' to answer this. I could order the majors by unemployment_rate to find which one has the lowest rate.
...
```{r rearrange_collge, message=TRUE}
college_recent_grads %>%
...
```{r lowest_unemploy, message=TRUE}
college_recent_grads %>%
  arrange(desc(unemployment_rate)) %>% 
...
**Response**: Nuclear Engineering is the major with the highest unemployment rate
...
```{r highest_prop_women, message=TRUE}
college_recent_grads %>%
...
**Response**: Early Childhood Education has the highest proportion of women.
...
```{r stem_low_salaries, message=TRUE}
college_recent_grads %>% 
